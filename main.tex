...
\begin{table}[H]
  \centering
  \caption{Rôle des optimiseurs comparés dans l'expérience}
  \label{tab:optimizer-role}
  \arrayrulecolor{black}
  \begin{tabular}{|p{2.5cm}|p{5.5cm}|p{6cm}|}
    \hline
    \textbf{Optimiseur} & \textbf{Principe principal} & \textbf{Objectif dans la comparaison} \\
    \hline
    SGD
      & Gradient stochastique simple
      & Référence de base \\
    \hline
    SGD + Momentum
      & Accumulation d'une direction de descente
      & Tester l'effet du momentum \\
    \hline
    Muon
      & Orthogonalisation du gradient
      & Étudier une méthode récente pour couches cachées \\
    \hline
    RMSProp
      & Adaptation par moyenne mobile des gradients
      & Tester une méthode adaptative stable \\
    \hline
    Adam
      & Momentum + adaptation du second moment
      & Comparer avec l'optimiseur adaptatif standard \\
    \hline
  \end{tabular}
\end{table}
